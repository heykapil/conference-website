...
export function TeamMembers() {
  ...
  useEffect(() => {
    ...
    if (innerScrollerRef2.current) {
      const scrollerContent = Array.from(
        innerScrollerRef2.current.children
      ) as HTMLElement[];
      scrollerContent.forEach((child) => {
        const duplicatedNode = child.cloneNode(true) as HTMLElement;
        duplicatedNode.setAttribute("aria-hidden", "true");
    ...
    if (innerScrollerRef3.current) {
      const scrollerContent = Array.from(
        innerScrollerRef3.current.children
      ) as HTMLElement[];
      scrollerContent.forEach((child) => {
        const duplicatedNode = child.cloneNode(true) as HTMLElement;
        duplicatedNode.setAttribute("aria-hidden", "true");
    ...
    if (innerScrollerRef4.current) {
      const scrollerContent = Array.from(
        innerScrollerRef4.current.children
      ) as HTMLElement[];
      scrollerContent.forEach((child) => {
        const duplicatedNode = child.cloneNode(true) as HTMLElement;
        duplicatedNode.setAttribute("aria-hidden", "true");
  ...
  return (
    <>
      <div className="bg-background">
        ...
        <section className={clsx("z-1 overflow-x-auto -mb-[1px] pb-[1px]")}>
          <div className="">
            <div className="container flex z-1 flex-col justify-center pt-[128px] lg:flex-row lg:items-center lg:pb-[148px]">
              <div className="flex z-1 flex-col items-center gap-6 pb-12 lg:items-start lg:pb-0">
                <div className="rounded-full bg-gradient-to-r from-[#31bdc6] to-[#3178c6] p-[1px] brightness-90 contrast-150 dark:brightness-125 dark:contrast-100">
                  <div className="rounded-full bg-white/80 px-3 py-1 dark:bg-black/80">
                    <span className="flex items-center bg-gradient-to-r from-[#31bdc6] to-[#3178c6] bg-clip-text text-transparent z-2">
                      {/* <GitBranch className="h-4 w-4 stroke-[#31bdc6] stroke-2 sm:mr-2" /> */}
                      <span className="hidden md:block z-1">
                ...
                <p className="max-w-[55ch] bg-transparent px-8 text-center leading-8 text-black/50 dark:text-white/50 lg:px-0 lg:text-left">
                  TypeHero is free, open-source, and built by developers just
                  like you. These are some of the members who made this possible
                  so far.
                </p>
              ...
              <div className="hidden flex-1 pb-0 pl-6 xl:block">
                ...
                <div className="honeycomboverride ml-auto flex flex-wrap xl:w-[38rem] ">
                  {members.map((contributor) => (
                    <Team member={contributor} key={contributor.id} />
                  ))}
                </div>
              </div>
              {/* autoscrolled */}
              <div className="pb-16 sm:pb-24 lg:pb-0 lg:pl-8 xl:hidden">
                <div className="infinite-scroll-x-container mx-auto hover:shadow-[0_0_10rem_10rem_#fff8] dark:hover:shadow-[0_0_10rem_10rem_#0008] lg:w-[25rem] xl:w-[35rem]">
                  <div
                    ...
                    className="infinite-scroll-x flex w-max flex-nowrap py-1.5"
                  >
                    {members.slice(0, members.length / 4).map((contributor) => (
                      <Team member={contributor} key={contributor.id} />
                    ))}
                  </div>
                </div>
                <div className="infinite-scroll-x-container mx-auto hover:shadow-[0_0_10rem_10rem_#fff8] dark:hover:shadow-[0_0_10rem_10rem_#0008] lg:w-[25rem] xl:w-[35rem]">
                  <div
                    ...
                    className="infinite-scroll-x-reverse  flex w-max flex-nowrap py-1.5"
                  >
                    {members
                      .slice(members.length / 4, (members.length / 4) * 2)
                      .map((contributor) => (
                        <Team member={contributor} key={contributor.id} />
                      ))}
                  </div>
                ...
                <div className="infinite-scroll-x-container mx-auto hover:shadow-[0_0_10rem_10rem_#fff8] dark:hover:shadow-[0_0_10rem_10rem_#0008] lg:w-[25rem] xl:w-[35rem]">
                  ...
                  >
                    {members
                      .slice((members.length / 4) * 2, (members.length / 4) * 3)
                      .map((contributor) => (
                        <Team member={contributor} key={contributor.id} />
                      ))}
                  </div>
                ...
                <div className="infinite-scroll-x-container mx-auto hover:shadow-[0_0_10rem_10rem_#fff8] dark:hover:shadow-[0_0_10rem_10rem_#0008] lg:w-[25rem] xl:w-[35rem]">
                  <div
                    ...
                    className="infinite-scroll-x-reverse  flex z-1 w-max flex-nowrap py-1.5"
                  >
                    {members
                      .slice((members.length / 4) * 3, members.length)
                      .map((contributor) => (
                        <Team member={contributor} key={contributor.id} />
                      ))}
                  </div>
